# The same as finetuning_samesample_time_schedule.py 
# BUUUUUUUUUUUUUUT -> instead of using a weighting function, we change the time-step sampling strategy -> SAME METHOD 
...
noise_schedule="cosine" # Time-step weighting (Changing this from cosine potentially)
use_kl=False
...
for dataset_size in [10]:#, 100, 700, 2503]:
    ...
    # time-schedule based on p2 weighting  time-step weighting
    for p2_gamma in [0, 0.1, 0.3, 1, 3, 10]:
        ...
        for g, g_name in {
            ...
            if g == 0 and p2_gamma in [0.1, 0.3, 1, 3, 10]: # For time-step weighting -> (SCHEDULE 2 STRATEGY) because p2_gamma will be multiplied by g=0 which yields 0 for every p2_gamma value
                continue
            ...
            diffusion = create_gaussian_diffusion(
                ...
                p2_gamma=0, # For time-step weighting -> HERE (SCHEDULE 2 STRATEGY) WE WILL NOT CHANGE THE DIFFUSION'S TIME-STEP BASED LOSS WEIGHTS, INSTEAD WE ARE CHANGING THE CLASSIFIER-FREE WEIGHTS
                p2_k=1, # For time-step weighting
            )

            print("*"*20)
            print(f"p2_gamma is {p2_gamma}")
            print("*"*20)
            ...
            clf_time_based = True # For time-step weighting (SCHEDULE 2 STRATEGY) 

            # Imagine we are training for 200 epochs max 
            # Fixed
            # ___________________________________ (SCHEDULE 2 STRATEGY) _________________________________________

            p2_gamma=p2_gamma # For time-step weighting
            p2_k=1, # For time-step weighting
            snr = 1.0 / (1 - diffusion.alphas_cumprod) - 1
            weight = 1 / (p2_k + snr)**p2_gamma

            guidance_scale = g * weight # For time-step weighting (SCHEDULE 2 STRATEGY) 

            # print('--'*20)
            # print('guidance scale is like this now')
            # print(guidance_scale[:20])
            # print(guidance_scale[-20:])
            ...
            loss_logger=f"/home/ymbahram/scratch/clf_trg_results/results_samesample/time-step_g-only/data{dataset_size}/{g_name}_{p2_gamma}/trainlog.csv"
            # If evaluation is true during training, where to save the FID stuff
            eval_logger=f"/home/ymbahram/scratch/clf_trg_results/results_samesample/time-step_g-only/data{dataset_size}/{g_name}_{p2_gamma}/evallog.csv"
            # Directory to save checkpoints in
            checkpoint_dir = f"/home/ymbahram/scratch/clf_trg_results/results_samesample/time-step_g-only/data{dataset_size}/{g_name}_{p2_gamma}/checkpoints/"
            # Whenever you are saving checkpoints, a batch of images are also sampled, where to produce these images
            save_samples_dir= f"/home/ymbahram/scratch/clf_trg_results/results_samesample/time-step_g-only/data{dataset_size}/{g_name}_{p2_gamma}/samples/"
            ...
            TrainLoop(
                ...
                clf_time_based=clf_time_based, #_____________ For time-step weighting (SCHEDULE 2 STRATEGY) _________!!!!!!!!!
                # for fixed sampling
